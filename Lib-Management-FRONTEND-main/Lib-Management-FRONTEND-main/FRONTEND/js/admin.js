document.addEventListener("DOMContentLoaded", function () {
    loadBooks();
    loadMembers();
    loadIssuedBooks(); // Load issued books in Admin Dashboard
});
...
function loadMembers() {
    ...
    members.forEach((member, index) => {
        let row = `<tr>
            <td>${member.email}</td>
            <td><button class="delete-btn" onclick="removeMember(${index})">Remove</button></td>
        </tr>`;
        tableBody.innerHTML += row;
    });
}
...
// **🔹 Remove Member**
// **🔹 Remove Member and Their Issued Books**
function removeMember(index) {
    let members = JSON.parse(localStorage.getItem("members")) || [];
    let issuedBooks = JSON.parse(localStorage.getItem("issuedBooks")) || [];

    let removedMemberEmail = members[index].email; // Get email of removed member

    // Remove the member from the list
    members.splice(index, 1);
    localStorage.setItem("members", JSON.stringify(members));

    // Remove all issued books linked to this member
    let filteredIssuedBooks = issuedBooks.filter(book => book.memberEmail !== removedMemberEmail);
    localStorage.setItem("issuedBooks", JSON.stringify(filteredIssuedBooks));

    alert("Member and their issued books removed successfully!");

    // Refresh UI
    loadMembers();  
    loadIssuedBooks();  
    loadBooks(); // 🔹 Update book availability
    loadAdminDashboardStats(); // 🔹 Update stats
}
...
function loadIssuedBooks() {
    ...
    issuedBooks.forEach((entry, index) => {
        let row = `<tr>
            ...
            <td>${entry.dueDate}</td>
            <td><button class="return-btn" onclick="adminReturnBook(${index})">Mark as Returned</button></td>
        </tr>`;
        tableBody.innerHTML += row;
    });
}
...
function adminReturnBook(index) {
    let issuedBooks = JSON.parse(localStorage.getItem("issuedBooks")) || [];
    ...
    loadIssuedBooks();
    loadBooks();
    loadAdminDashboardStats();  // 🔹 Update admin stats
}
